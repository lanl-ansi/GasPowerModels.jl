...
@testset "test neopf" begin
    ...
    @testset "Pressure Penalty" begin
        ...
        @test isapprox(result["objective"],  484124; atol = 1e0) || isapprox(result["objective"],  484132; atol = 1e0)
    end
    ...
    @testset "Demand Penalty" begin
        ...
        @test result["status"] == :LocalOptimal || result["status"] == :Optimal
        @test isapprox(result["objective"],  250139; atol = 1.0) || isapprox(result["objective"],  248541; atol = 2.0) 
    end
 end
