...
@testset "test neopf" begin
    @testset "Pressure Penalty" begin
        result = GasPowerModels.run_ne_opf("../test/data/no_cost.m", "../test/data/no_demand_cost.json", SOCWRPowerModel, MISOCPGasModel, misocp_solver)
    ...
    @testset "Demand Penalty" begin
        result = GasPowerModels.run_ne_opf("../test/data/no_cost.m", "../test/data/no_pressure_cost.json", SOCWRPowerModel, MISOCPGasModel, misocp_solver)
        @test result["status"] == :LocalOptimal || result["status"] == :Optimal
        println(result["objective"])
        # Tested with Gurobi 8.1
