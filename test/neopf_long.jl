...
#    @testset "36 Bus Ilic 1.1 Northeast 2.25" begin
 #       result = GasGridModels.run_ne_opf("../data/36bus_ilic_expansion_1.1.json", "../data/TC_PennToNortheast_wValves_expansion_2.25.json", SOCWRPowerModel, MISOCPGasModel, misocp_solver; power_opf_weight=365, gas_price_weight=365)
  #      @test result["status"] == :LocalOptimal || result["status"] == :Optimal
   #     @test isapprox(result["objective"], 5.127290418071447e9; atol = 1e5) 
   # end

    @testset "36 Bus Ilic 1.1 Northeast 4.0" begin
        result = GasGridModels.run_ne_opf("../data/36bus_ilic_expansion_1.1.json", "../data/TC_PennToNortheast_wValves_expansion_4.0.json", SOCWRPowerModel, MISOCPGasModel, misocp_solver; power_opf_weight=365, gas_price_weight=365)
        @test result["status"] == :LocalOptimal || result["status"] == :Optimal
