...
function solve_ne_opf(power_file, gas_file, power_model_constructor, gas_model_constructor, solver; solution_builder=get_ne_opf_solution, kwargs...)
    return solve_generic_model(power_file, gas_file, power_model_constructor, gas_model_constructor, solver, post_ne_opf; power_ref_extensions=[_PM.ref_add_on_off_va_bounds!,_PM.ref_add_ne_branch!], solution_builder=solution_builder, kwargs...)
end

" Construct the gas flow feasbility problem with demand being the cost model"
function post_ne_opf(pm::_PM.AbstractPowerModel, gm::_GM.AbstractGasModel; kwargs...)
    kwargs = Dict(kwargs)
...
function get_ne_opf_solution(pm::_PM.AbstractPowerModel, gm::_GM.AbstractGasModel)
...
function add_zone_cost_setpoint(sol, gm::_GM.AbstractGasModel)
