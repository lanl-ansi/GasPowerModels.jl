...
function variable_zone_demand(gm::GenericGasModel, n::Int=gm.cnw)
    ...
    gm.var[:nw][n][:zone_fl] = @variable(gm.model, [i in keys(gm.ref[:nw][n][:price_zone])], basename="$(n)_zone_fl", lowerbound=0.0, upperbound=flmax[i])
end

" function for creating variables associated with zonal demand "
function variable_zone_demand_price(gm::GenericGasModel, n::Int=gm.cnw)
    gm.var[:nw][n][:zone_cost] = @variable(gm.model, [i in keys(gm.ref[:nw][n][:price_zone])], basename="$(n)_zone_cost", lowerbound=0.0, upperbound=Inf)
end
...
function variable_zone_pressure(gm::GenericGasModel, n::Int=gm.cnw)
    ...
    gm.var[:nw][n][:zone_p] = @variable(gm.model, [i in keys(gm.ref[:nw][n][:price_zone])], basename="$(n)_zone_p", lowerbound=pmin[i], upperbound=pmax[i])
end
...
function variable_pressure_price(gm::GenericGasModel, n::Int=gm.cnw)
    ...
    gm.var[:nw][n][:p_cost] = @variable(gm.model, [i in keys(gm.ref[:nw][n][:price_zone])], basename="$(n)_p_cost", lowerbound=max(0,cmin[i]), upperbound=cmax[i])
end
