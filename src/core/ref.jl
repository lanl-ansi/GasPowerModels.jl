...
function ref_add_price_zones!(ref::Dict{Symbol, <:Any}, data::Dict{String, <:Any})
    nws_data = _IM.ismultinetwork(data["it"][_GM.gm_it_name]) ? data["it"][_GM.gm_it_name]["nw"] : Dict("0" => data["it"][_GM.gm_it_name])
    q_base, p_base = Float64(ref[:it][_GM.gm_it_sym][:base_flow]), Float64(ref[:it][_GM.gm_it_sym][:base_pressure])
