...
function constraint_zone_demand(gm::GenericGasModel, n::Int, i, loads)
    ...
    _GM.add_constraint(gm, n, :zone_demand, i, @constraint(gm.model, zone_fl[i] == sum(fl[j] for j in loads)))
end
...
function constraint_zone_demand_price(gm::GenericGasModel, n::Int, i, min_cost, cost_q, standard_density)
    ...
    _GM.add_constraint(gm, n, :zone_demand_price1, i, @constraint(gm.model, zone_cost[i] >= 86400^2 * cost_q[1] * (zone_fl[i] / standard_density)^2 + 86400 * cost_q[2] * zone_fl[i] / standard_density + cost_q[3]))
    _GM.add_constraint(gm, n, :zone_demand_price2, i, @constraint(gm.model, zone_cost[i] >= 86400 * min_cost * zone_fl[i] / standard_density))
end
...
function constraint_pressure_price(gm::GenericGasModel, n::Int, i, cost_p)
    ...
    _GM.add_constraint(gm, n, :pressure_price, i, @constraint(gm.model, p_cost[i] >= cost_p[1] * zone_p[i]^2 + cost_p[2] * zone_p[i] + cost_p[3]))
end
